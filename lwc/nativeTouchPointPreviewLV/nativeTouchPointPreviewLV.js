...
import Published_Label from '@salesforce/label/c.Published_Label';
...
export default class NativeTouchPointPreviewLV extends NavigationMixin(LightningElement) {
    ...
    get permissionOptions() {
        return [
            { label: this.label.Public_Permission, value: 'public' },
    ...
    get statusOptions() {
        return [
            ...
            { label: this.label.Published_Label, value: "published" },
            ...
            .then(result => {
                ...
                Object.keys(mapTemplates).forEach(key => {
                    mapTemplates[key].forEach(ele => {
                        ...
                        if (key == 'private' && ele.status.toLowerCase() == 'published') {
                ...
                console.log('VOD ----------------------- mapTemplates[this.listViewValue] ', mapTemplates[this.listViewValue]);
    ...
    setTemplateBagdeColor(template){
        ...
        if (template.status.toLowerCase() == 'drafted') {
            ...
            template.statusLabel = this.label.Published_Label;
            template.statusCSS = '';

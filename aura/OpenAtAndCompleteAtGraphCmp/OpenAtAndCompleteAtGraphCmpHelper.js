            ...
            if(!$A.util.isEmpty(dataObj) ){
                ...
                for(let i = 0 ; i < dataObj.length ; i++){
                    ...
                    for(let j =0 ; j < dataObj[i].dataWrapperList.length ; j++){
                        ...
                        let lenghtofOpenAndCompleted = dataObj[i].dataWrapperList[j].Data;
                        if(!mapObj.has(openAndCompleted)){
                            mapObj.set(openAndCompleted, lenghtofOpenAndCompleted);
                        }
                        maximumList = lenghtofOpenAndCompleted;
                    }
            ...
            Chart.plugins.register({
                afterDraw: function(chart) {
                    if (chart.data.datasets.length === 0) {
                        ...
                        wrapText(ctx, $A.get("$Label.c.Campaign_Graph_Will_Populate_Text"), width / 2, height / 2, width, 20 * 1.618);
                            ...
                            wrapText(ctx, $A.get("$Label.c.Campaign_Graph_Send_Touchpoint_Text"), width / 2, height / 2, width, 20 * 1.618);
                        }
                        ctx.restore();
                    }
                }

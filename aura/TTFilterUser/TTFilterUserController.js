  ...
  viewRecord: function (c, e, h) {
    ...
    if(fieldName == "Create Task"){
      ...
      if (!$A.util.isEmpty(recId)) {
        ...
        console.log('VOD ---------------------- sfId ',sfId);
        console.log('VOD ---------------------- sfId.completedAndSync ',sfId.completedAndSync);
        console.log('VOD ---------------------- !sfId.completedAndSync ',!sfId.completedAndSync);
        if(!sfId.completedAndSync){
          console.log('VOD ---------------------- handleInsightCallout ');
          h.handleInsightCallout(c, recId);
        } else {
          console.log('VOD ---------------------- viewRecord_helper ');
